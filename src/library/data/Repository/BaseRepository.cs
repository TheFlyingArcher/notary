    ...
    where TM : BaseModel, new()
{
    ...
    public IMongoCollection<TM> Collection { get; }
    ...
    public virtual async Task SaveAsync(TC entity)
    {
        ...
        if (string.IsNullOrEmpty(entity.Slug)) entity.Slug = entity.Slugify();
    ...
    protected string GetCollectionName()
    {
        ...
        var modelName = modelType.Name.Replace("Model", string.Empty);
        var collectionName = string.Empty;
        var endingCharacter = modelName[modelName.Length - 1];

        //Be smart about English
        if (char.ToLowerInvariant(endingCharacter) == 'y')
        {
            var nextChar = modelName[modelName.Length - 2];
            char[] vowels = { 'a', 'e', 'i', 'o', 'u' };
            if (vowels.Any(v => v == nextChar))
                collectionName = string.Concat(modelName, 's'); //bays, toys, keys
            else
                collectionName = string.Concat(modelName[..^1], "ies"); //histories, flies, countries, etc.
        }
